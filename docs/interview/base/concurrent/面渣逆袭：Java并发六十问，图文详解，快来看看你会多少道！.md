...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_48c003ac-742b-11ec-9ca7-fa163eb4f6be.png)
并行和并发

就好像我们去食堂打饭，并行就是我们在多个窗口排队，几个阿姨同时打菜；并发就是我们挤在一个窗口，阿姨给这个打一勺，又手忙脚乱地给那个打一勺。

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_48dec2e2-742b-11ec-9ca7-fa163eb4f6be.png)
并行并发和食堂打饭
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_490ff07e-742b-11ec-9ca7-fa163eb4f6be.png)
程序进程线程关系
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4939a18a-742b-11ec-9ca7-fa163eb4f6be.png)
线程创建三种方式
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_495cd7c2-742b-11ec-9ca7-fa163eb4f6be.png)
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_496fd354-742b-11ec-9ca7-fa163eb4f6be.png)
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4985beee-742b-11ec-9ca7-fa163eb4f6be.png)
Java线程状态变化
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_49b6e8ca-742b-11ec-9ca7-fa163eb4f6be.png)
线程切换-2020-12-16-2107

为了让用户感觉多个线程是在同时执行的， CPU 资源的分配采用了时间片轮转也就是给每个线程分配一个时间片，线程在时间片内占用 CPU 执行任务。当线程使用完时间片后，就会处于就绪状态并让出 CPU 让其他线程占用，这就是上下文切换。

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_49d3bb9e-742b-11ec-9ca7-fa163eb4f6be.png)
上下文切换时机
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_49f6e4a2-742b-11ec-9ca7-fa163eb4f6be.png)
线程间通信方式
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4a0cb674-742b-11ec-9ca7-fa163eb4f6be.png)
ThreadLocal线程副本
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4a303d06-742b-11ec-9ca7-fa163eb4f6be.png)
ThreadLoca存放用户上下文
...
我们看一下ThreadLocal的set(T)方法，发现先获取到当前线程，再获取`ThreadLocalMap`
，然后把元素存到这个map中。
...
ThreadLocal实现的秘密都在这个`ThreadLocalMap`
了，可以Thread类中定义了一个类型为`ThreadLocal.ThreadLocalMap`
的成员变量`threadLocals`
。
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4a4d295c-742b-11ec-9ca7-fa163eb4f6be.png)
ThreadLoca结构图
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4a6cfd72-742b-11ec-9ca7-fa163eb4f6be.png)
ThreadLocal内存分配
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4aa047d6-742b-11ec-9ca7-fa163eb4f6be.png)
ThreadLocalMap结构示意图
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4acb1d80-742b-11ec-9ca7-fa163eb4f6be.png)
ThreadLocalMap解决冲突
...
接着看看具体的`resize()`
方法，扩容后的`newTab`
的大小为老数组的两倍，然后遍历老的table数组，散列方法重新计算位置，开放地址解决冲突，然后放到新的`newTab`
，遍历完成之后，`oldTab`
中所有的`entry`
数据都已经放入到`newTab`
中了，然后table引用指向`newTab`

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4b0cb0a6-742b-11ec-9ca7-fa163eb4f6be.png)
ThreadLocalMap扩容

具体代码：

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4b49c176-742b-11ec-9ca7-fa163eb4f6be.png)
...
在Thread.init的时候，如果父线程的`inheritableThreadLocals`
不为空，就把它赋给当前线程（子线程）的`inheritableThreadLocals`
。
...
JMM定义了线程和主内存之间的抽象关系：线程之间的共享变量存储在`主内存`
（Main Memory）中，每个线程都有一个私有的`本地内存`
（Local Memory），本地内存中存储了该线程以读/写共享变量的副本。

Java内存模型的抽象图：

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4b752636-742b-11ec-9ca7-fa163eb4f6be.png)
Java内存模型

本地内存是JMM的 一个抽象概念，并不真实存在。它其实涵盖了缓存、写缓冲区、寄存器以及其他的硬件和编译器优化。

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4bb638ec-742b-11ec-9ca7-fa163eb4f6be.png)
实际线程工作模型
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4be1563a-742b-11ec-9ca7-fa163eb4f6be.png)
多级指令重排
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4c16f362-742b-11ec-9ca7-fa163eb4f6be.png)
双重校验单例模式异常情形
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4c37b070-742b-11ec-9ca7-fa163eb4f6be.png)
happens-before六大规则
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4c661686-742b-11ec-9ca7-fa163eb4f6be.png)
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4c8fbd10-742b-11ec-9ca7-fa163eb4f6be.png)
两种执行结果
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4ca5ebbc-742b-11ec-9ca7-fa163eb4f6be.png)
volatile内存可见性
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4cbfdc52-742b-11ec-9ca7-fa163eb4f6be.png)
volatile重排序规则表

为了实现volatile的内存语义，编译器在生成字节码时，会在指令序列中插入内存屏障来禁止特定类型的处理器重排序。

1. 在每个volatile写操作的前面插入一个`StoreStore`
   屏障
2. 在每个volatile写操作的后面插入一个`StoreLoad`
   屏障
3. 在每个volatile读操作的后面插入一个`LoadLoad`
   屏障
4. 在每个volatile读操作的后面插入一个`LoadStore`
   屏障

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4cf53bfe-742b-11ec-9ca7-fa163eb4f6be.png)
volatile写插入内存屏障后生成的指令序列示意图

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4d1596a6-742b-11ec-9ca7-fa163eb4f6be.png)
volatile写插入内存屏障后生成的指令序列示意图
...
1. synchronized修饰代码块时，JVM采用`monitorenter`
   、`monitorexit`
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4d49fbf8-742b-11ec-9ca7-fa163eb4f6be.png)
monitorenter和monitorexit

1. synchronized修饰同步方法时，JVM采用`ACC_SYNCHRONIZED`
   标记符来实现同步，这个标识指明了该方法是一个同步方法。

   同样可以写段代码反编译看一下。

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4d8e67de-742b-11ec-9ca7-fa163eb4f6be.png)
synchronized修饰同步方法
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4dc35eee-742b-11ec-9ca7-fa163eb4f6be.png)
就诊-图片来源参考[18]
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4e0257f2-742b-11ec-9ca7-fa163eb4f6be.png)
Java Montior机制
...
Java对象头里，有一块结构，叫`Mark Word`
标记字段，这块结构会随着锁的状态变化而变化。

64 位虚拟机 Mark Word 是 64bit，我们来看看它的状态变化：

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4e3777c0-742b-11ec-9ca7-fa163eb4f6be.png)
Mark Word变化
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4e598ef0-742b-11ec-9ca7-fa163eb4f6be.png)
锁升级方向
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4e7b0b7a-742b-11ec-9ca7-fa163eb4f6be.png)
锁升级简略过程

完整的升级过程：

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4eb9fc68-742b-11ec-9ca7-fa163eb4f6be.png)
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4ee41b92-742b-11ec-9ca7-fa163eb4f6be.png)
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4f0f812e-742b-11ec-9ca7-fa163eb4f6be.png)
先简单了解一下CLH：Craig、Landin and Hagersten 队列，是 **单向链表实现的队列**。申请线程只在本地变量上自旋，**它不断轮询前驱的状态**，如果发现 **前驱节点释放了锁就结束自旋**

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4f241e86-742b-11ec-9ca7-fa163eb4f6be.png)
CLH队列

AQS 中的队列是 CLH 变体的虚拟双向队列，通过将每条请求共享资源的线程封装成一个节点来实现锁的分配：

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4f7063cc-742b-11ec-9ca7-fa163eb4f6be.png)
AQS变种CLH队列
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4f8ad900-742b-11ec-9ca7-fa163eb4f6be.png)
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4faa4bc8-742b-11ec-9ca7-fa163eb4f6be.png)
公平锁tryAcquire
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_4ff00dc0-742b-11ec-9ca7-fa163eb4f6be.png)
CAS三大问题
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_501b2bb8-742b-11ec-9ca7-fa163eb4f6be.png)
Java保证原子性方法
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_50440380-742b-11ec-9ca7-fa163eb4f6be.png)
原子操作类
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_5067c20c-742b-11ec-9ca7-fa163eb4f6be.png)
死锁示意图

那么为什么会产生死锁呢？死锁的产生必须具备以下四个条件：

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_50888f50-742b-11ec-9ca7-fa163eb4f6be.png)
死锁产生必备四条件
...
还可以利用图形化工具，比如JConsole。出现线程死锁以后，点击JConsole线程面板的`检测到死锁`
按钮，将会看到线程的死锁信息。

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_50b2fd8a-742b-11ec-9ca7-fa163eb4f6be.png)
线程死锁检测
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_50d4de8c-742b-11ec-9ca7-fa163eb4f6be.png)
王者荣耀等待玩家确认-来源参考[18]
...
用户创建初始的latch数量，各玩家通过`countDownLatch.countDown()`
完成状态确认，主线程通过`countDownLatch.await()`
等待。
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_50f05eb4-742b-11ec-9ca7-fa163eb4f6be.png)
王者荣耀-来源参考[18]

在这个场景中，仍然用五个线程代表大乔、兰陵王、安其拉、哪吒和铠等五个玩家。需要注意的是，各玩家虽然都调用了`start()`
线程，但是它们在运行时都在等待`countDownLatch`
...
- `boolean await(long timeout, TimeUnit unit)`
  ：等待latch降为0，但是可以设置超时时间。比如有玩家超时未确认，那就重新匹配，总不能为了某个玩家等到天荒地老。
- `countDown()`
  ：latch数量减1；
- `getCount()`
  ：获取当前的latch数量。
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_51221ff8-742b-11ec-9ca7-fa163eb4f6be.png)
想结衣结衣不依,迷爱理爱理不理。

我们拿代码模拟这一场景，发现CountDownLatch无能为力了，因为CountDownLatch的使用是一次性的，无法重复利用，而这里等待了两次。此时，我们用CyclicBarrier就可以实现，因为它可以重复利用。

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_51746416-742b-11ec-9ca7-fa163eb4f6be.png)
小约翰可汗选妃模拟代码

运行结果：

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_519f0702-742b-11ec-9ca7-fa163eb4f6be.png)
运行结果
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_51c8aefe-742b-11ec-9ca7-fa163eb4f6be.png)
CyclicBarrier工作流程
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_51e540e6-742b-11ec-9ca7-fa163eb4f6be.png)
停车场空闲车位提示-图片来源网络

我们把这个例子类比一下，车辆就是线程，进入停车场就是线程在执行，离开停车场就是线程执行完毕，看见红灯就表示线程被阻塞，不能执行，Semaphore的本质就是**协调多个线程对共享资源的获取**。

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_520103a8-742b-11ec-9ca7-fa163eb4f6be.png)
...
表示允许10个线程获取许可证，也就是最大并发数是10。Semaphore的用法也很简单，首先线程使用 Semaphore的acquire()方法获取一个许可证，使用完之后调用release()方法归还许可证。还可以用tryAcquire()方法尝试获取许可证。

## 43.Exchanger 了解吗？

Exchanger（交换者）是一个用于线程间协作的工具类。Exchanger用于进行线程间的数据交换。它提供一个同步点，在这个同步点，两个线程可以交换彼此的数据。

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_5224676c-742b-11ec-9ca7-fa163eb4f6be.png)
英雄交换猎物-来源参考[18]
...
假如两个线程有一个没有执行exchange()方法，则会一直等待，如果担心有特殊情况发生，避免一直等待，可以使用`exchange(V x, long timeOut, TimeUnit unit)`
设置最大等待时长。
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_5237456c-742b-11ec-9ca7-fa163eb4f6be.png)
管理线程的池子
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_525e3b86-742b-11ec-9ca7-fa163eb4f6be.png)
业务示例

主要代码如下：

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_5272894c-742b-11ec-9ca7-fa163eb4f6be.png)
主要代码
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_529e4a78-742b-11ec-9ca7-fa163eb4f6be.png)
直接办理

1. 老三发现没有空闲的窗口，就在排队区排队等。

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_52c20aa8-742b-11ec-9ca7-fa163eb4f6be.png)
排队等待

1. 老三发现没有空闲的窗口，等待区也满了，蚌埠住了，经理一看，就让休息的小姐姐赶紧回来上班，等待区号靠前的赶紧去新窗口办，老三去排队区排队。小姐姐比较辛苦，假如一段时间发现他们可以不用接着营业，经理就让她们接着休息。

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_52deef7e-742b-11ec-9ca7-fa163eb4f6be.png)
排队区满

1. 老三一看，六个窗口都满了，等待区也没位置了。老三急了，要闹，经理赶紧出来了，经理该怎么办呢？

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_530f3706-742b-11ec-9ca7-fa163eb4f6be.png)
等待区，排队区都满
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_533aa51c-742b-11ec-9ca7-fa163eb4f6be.png)
线程池执行流程
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_535194b6-742b-11ec-9ca7-fa163eb4f6be.png)
线程池参数

线程池有七大参数，需要重点关注`corePoolSize`
、`maximumPoolSize`
、`workQueue`
、`handler`
这四个。
...
`maximumPoolSize`
表示允许的最大线程数 = (非核心线程数+核心线程数)，当`BlockingQueue`
...
线程池等待队列，维护着等待执行的`Runnable`
对象。当运行当线程数= corePoolSize时，新的任务会被添加到`workQueue`
中，如果`workQueue`
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_5389892a-742b-11ec-9ca7-fa163eb4f6be.png)
四种策略
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_53a44698-742b-11ec-9ca7-fa163eb4f6be.png)
线程池常用阻塞队列
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_53cdae52-742b-11ec-9ca7-fa163eb4f6be.png)
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_53eef5c6-742b-11ec-9ca7-fa163eb4f6be.png)
四大线程池
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_540c8910-742b-11ec-9ca7-fa163eb4f6be.png)
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_54235ece-742b-11ec-9ca7-fa163eb4f6be.png)
FixedThreadPool
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_5444316c-742b-11ec-9ca7-fa163eb4f6be.png)
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_5462a53e-742b-11ec-9ca7-fa163eb4f6be.png)
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_547aa1ac-742b-11ec-9ca7-fa163eb4f6be.png)
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_54a5e7ea-742b-11ec-9ca7-fa163eb4f6be.png)
线程池异常处理
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_54be3304-742b-11ec-9ca7-fa163eb4f6be.png)
线程池状态切换图
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_54f1d95c-742b-11ec-9ca7-fa163eb4f6be.png)
JDK 线程池参数设置接口来源参考[7]

这里主要有两个思路：

![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_551fa15c-742b-11ec-9ca7-fa163eb4f6be.png)
动态修改线程池参数
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_556989de-742b-11ec-9ca7-fa163eb4f6be.png)
线程池调优
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_5590f2ee-742b-11ec-9ca7-fa163eb4f6be.png)
线程池主要实现流程
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_55acb1be-742b-11ec-9ca7-fa163eb4f6be.png)
自定义线程池
...
我们可以对正在处理和阻塞队列的任务做事务管理或者对阻塞队列中的任务持久化处理，并且当断电或者系统崩溃，操作无法继续下去的时候，可以通过回溯日志的方式来撤销`正在处理`
...
关于一些并发容器，可以去看看 面渣逆袭：Java集合连环三十问  ，里面有`CopyOnWriteList`
和`ConcurrentHashMap`
这两种线程安全容器类的问答。。
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_55dc95fa-742b-11ec-9ca7-fa163eb4f6be.png)
Fork/Join分治算法
...
![img](https://oss-emcsprod-public.modb.pro/wechatSpider/modb_20220113_56033886-742b-11ec-9ca7-fa163eb4f6be.png)
工作窃取
...
- [11]. 讲真 这次绝对让你轻松学习线程池:https://mp.weixin.qq.com/s/dTMH1TdxiCKy5yotQ7u7cA
...
- [16]. 面试官：小伙子，听说你看过ThreadLocal源码？（万字图文深度解析ThreadLocal）:https://juejin.cn/post/6844904151567040519
