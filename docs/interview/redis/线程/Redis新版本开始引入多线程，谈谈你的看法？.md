...
- `文件事件`：Redis服务器通过套接字与客户端（或者其他Redis服务器）进行连接，而文件事件就是服务器对套接字操作的抽象；服务器与客户端的通信会产生相应的文件事件，而服务器则通过监听并处理这些事件来完成一系列网络通信操作，比如连接`accept`，`read`，`write`，`close`等；
  ...
  ![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/HV4yTI6PjbIB7Fc7j66tvicFghRd6Sibq8ibbtjOgxMw2DjJUWYMV6V5jnGZOJxodsl3KDbsnC1iadyhjXKQia0Qfug/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/HV4yTI6PjbIB7Fc7j66tvicFghRd6Sibq8DP4D77tZOHQVdlbPBWU0SlaErbwV7S32icdOMuIXnzWxXAvSRibNfQvA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/HV4yTI6PjbIB7Fc7j66tvicFghRd6Sibq8J2NEhD1nUUb85mUJa2Y6RDkDXqcTfoQRJagEn4p89vPf0uIrzC5Mdg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
